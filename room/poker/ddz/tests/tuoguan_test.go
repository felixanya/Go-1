...
func Test_Tuoguan_Single(t *testing.T) {
	...
	// 手中的牌（96652244）
	handCards := states.ToPokers([]uint32{0x29, 0x16, 0x26, 0x45, 0x22, 0x32, 0x24, 0x34})

	// 上次出的牌（3）
	lastCards := states.ToPokers([]uint32{0x33})
...
func Test_Tuoguan_Pair(t *testing.T) {
	...
	// 手中的牌（9665224）
	handCards := states.ToPokers([]uint32{0x29, 0x16, 0x26, 0x45, 0x22, 0x32, 0x24})

	// 上次出的牌（33）
	lastCards := states.ToPokers([]uint32{0x33, 0x43})
...
func Test_Tuoguan_Shunzi(t *testing.T) {
	...
	// 手中的牌（987665224）
	handCards := states.ToPokers([]uint32{0x29, 0x28, 0x27, 0x16, 0x26, 0x45, 0x22, 0x32, 0x24})

	// 上次出的牌（43567）
	lastCards := states.ToPokers([]uint32{0x34, 0x43, 0x45, 0x46, 0x47})
...
func Test_Tuoguan_Shunzi1(t *testing.T) {
	...
	// 手中的牌（987665224）
	handCards := states.ToPokers([]uint32{0x3A, 0x29, 0x28, 0x27, 0x16, 0x26, 0x22, 0x32, 0x24}) //2246678910
...
func Test_Tuoguan_Pairs(t *testing.T) {
	...
	// 手中的牌（10998877665224）
	handCards := states.ToPokers([]uint32{0x2A, 0x19, 0x29, 0x28, 0x18, 0x27, 0x17, 0x16, 0x26, 0x45, 0x22, 0x32, 0x24})

	// 上次出的牌（443355）
	lastCards := states.ToPokers([]uint32{0x34, 0x14, 0x43, 0x13, 0x45, 0x35})
...
func Test_Tuoguan_Triple(t *testing.T) {
	...
	// 手中的牌（999886665224）
	handCards := states.ToPokers([]uint32{0x19, 0x29, 0x39, 0x28, 0x18, 0x16, 0x26, 0x36, 0x45, 0x22, 0x32, 0x24})

	// 上次出的牌（444）
	lastCards := states.ToPokers([]uint32{0x34, 0x14, 0x44})
...
func Test_Tuoguan_3And1(t *testing.T) {
	...
	// 上次出的牌（4445）
	lastCards := states.ToPokers([]uint32{0x34, 0x15, 0x14, 0x44})
...
func Test_Tuoguan_3And2(t *testing.T) {
	...
	// 上次出的牌（44455）
	lastCards := states.ToPokers([]uint32{0x34, 0x15, 0x14, 0x25, 0x44})
...
func Test_Tuoguan_Triples(t *testing.T) {
	...
	// 手中的牌（9998886665224）
	handCards := states.ToPokers([]uint32{0x19, 0x29, 0x39, 0x28, 0x18, 0x38, 0x16, 0x26, 0x36, 0x45, 0x22, 0x32, 0x24})

	// 上次出的牌（444333）
	lastCards := states.ToPokers([]uint32{0x34, 0x14, 0x43, 0x44, 0x23, 0x13})
...
func Test_Tuoguan_3sAnd1s(t *testing.T) {
	...
	// 上次出的牌（4443339J）
	lastCards := states.ToPokers([]uint32{0x34, 0x14, 0x43, 0x44, 0x23, 0x13, 0x49, 0x1B})
...
func Test_Tuoguan_3sAnd1s_1(t *testing.T) {
	...
	handCards := states.ToPokers([]uint32{0x18, 0x28, 0x38, 0x48, 0x16, 0x26, 0x36, 0x46, 0x17, 0x27, 0x37, 0x47, 0x19, 0x29, 0x39, 0x49})

	// 上次出的牌（333344445555）
	lastCards := states.ToPokers([]uint32{0x13, 0x23, 0x33, 0x43, 0x14, 0x24, 0x34, 0x44, 0x15, 0x25, 0x35, 0x45})
...
func Test_Tuoguan_3sAnd2s(t *testing.T) {
	...
	// 上次出的牌（44433377JJ）
	lastCards := states.ToPokers([]uint32{0x34, 0x14, 0x43, 0x44, 0x23, 0x13, 0x17, 0x1B, 0x27, 0x2B})
...
func Test_Tuoguan_3sAnd2s_1(t *testing.T) {
	...
	// 手中的牌（6667778888）
	handCards := states.ToPokers([]uint32{0x16, 0x26, 0x36, 0x17, 0x27, 0x37, 0x18, 0x28, 0x38, 0x48})

	// 上次出的牌（3334445555）
	lastCards := states.ToPokers([]uint32{0x13, 0x23, 0x33, 0x14, 0x24, 0x34, 0x15, 0x25, 0x35, 0x45})
...
func Test_Tuoguan_3sAnd2s_2(t *testing.T) {
	...
	handCards := states.ToPokers([]uint32{0x19, 0x29, 0x39, 0x1A, 0x2A, 0x3A, 0x1B, 0x2B, 0x3B, 0x1C, 0x2C, 0x3C, 0x1D, 0x2D, 0x3D, 0x4D, 0x11, 0x21, 0x31, 0x41})

	// 上次出的牌（33344455566677778888）
	lastCards := states.ToPokers([]uint32{0x13, 0x23, 0x33, 0x14, 0x24, 0x34, 0x15, 0x25, 0x35, 0x16, 0x26, 0x36, 0x17, 0x27, 0x37, 0x47, 0x18, 0x28, 0x38, 0x48})
...
func Test_Tuoguan_4sAnd1s(t *testing.T) {
	...
	// 手中的牌（9988886666224）
	handCards := states.ToPokers([]uint32{0x19, 0x29, 0x48, 0x28, 0x18, 0x38, 0x16, 0x26, 0x36, 0x46, 0x22, 0x32, 0x24})

	// 上次出的牌（7777Q9）
	lastCards := states.ToPokers([]uint32{0x17, 0x1C, 0x27, 0x37, 0x47, 0x19})
...
func Test_Tuoguan_4sAnd2s(t *testing.T) {
	...
	// 上次出的牌（77779933）
	lastCards := states.ToPokers([]uint32{0x17, 0x27, 0x37, 0x47, 0x39, 0x39, 0x33, 0x23})
...
func Test_Tuoguan_4sAnd2s_1(t *testing.T) {
	...
	// 手中的牌（88887777）
	handCards := states.ToPokers([]uint32{0x48, 0x28, 0x18, 0x38, 0x17, 0x27, 0x37, 0x47})

	// 上次出的牌（33334444）
	lastCards := states.ToPokers([]uint32{0x13, 0x23, 0x33, 0x43, 0x14, 0x24, 0x34, 0x44})
...
func Test_Tuoguan_Boom(t *testing.T) {
	...
	// 上次出的牌（7777）
	lastCards := states.ToPokers([]uint32{0x17, 0x27, 0x37, 0x47})
...
func Test_Tuoguan(t *testing.T) {
	handCards := states.ToPokers([]uint32{0x22, 0x31, 0x4D, 0x4C, 0x3C, 0x1C, 0x48, 0x38, 0x17, 0x36, 0x35, 0x25, 0x34, 0x24, 0x43, 0x33, 0x13}) //♣2 ♥A ♠K ♠J ♥J ♦J ♠8 ♥8 ♦7 ♥6 ♥5 ♣5 ♥4 ♣4 ♠3 ♥3 ♦3
	lastCards := states.ToPokers([]uint32{0x3B, 0x3A, 0x2A, 0x1A, 0x49, 0x39, 0x29, 0x15})                                                       //♣J ♥10 ♣10 ♦10 ♠9 ♥9 ♣9 ♦5
