// Code generated by protoc-gen-go. DO NOT EDIT.
// source: phone.proto

package hall // import "steve/client_pb/hall"

import proto "github.com/golang/protobuf/proto"
import fmt "fmt"
import math "math"
import common "steve/client_pb/common"

// Reference imports to suppress errors if they are not otherwise used.
var _ = proto.Marshal
var _ = fmt.Errorf
var _ = math.Inf

// This is a compile-time assertion to ensure that this generated file
// is compatible with the proto package it is being compiled against.
// A compilation error at this line likely means your copy of the
// proto package needs to be updated.
const _ = proto.ProtoPackageIsVersion2 // please upgrade the proto package

// 验证码发送场景
type AuthCodeSendScene int32

const (
	AuthCodeSendScene_REGISTER        AuthCodeSendScene = 1
	AuthCodeSendScene_RESET_PASSWORD  AuthCodeSendScene = 2
	AuthCodeSendScene_RESET_CELLPHONE AuthCodeSendScene = 3
	AuthCodeSendScene_NEW_HALL_LOGIN  AuthCodeSendScene = 4
	AuthCodeSendScene_BIND_PHONE      AuthCodeSendScene = 5
	AuthCodeSendScene_BIND_WECHAT     AuthCodeSendScene = 10
)

var AuthCodeSendScene_name = map[int32]string{
	1:  "REGISTER",
	2:  "RESET_PASSWORD",
	3:  "RESET_CELLPHONE",
	4:  "NEW_HALL_LOGIN",
	5:  "BIND_PHONE",
	10: "BIND_WECHAT",
}
var AuthCodeSendScene_value = map[string]int32{
	"REGISTER":        1,
	"RESET_PASSWORD":  2,
	"RESET_CELLPHONE": 3,
	"NEW_HALL_LOGIN":  4,
	"BIND_PHONE":      5,
	"BIND_WECHAT":     10,
}

func (x AuthCodeSendScene) Enum() *AuthCodeSendScene {
	p := new(AuthCodeSendScene)
	*p = x
	return p
}
func (x AuthCodeSendScene) String() string {
	return proto.EnumName(AuthCodeSendScene_name, int32(x))
}
func (x *AuthCodeSendScene) UnmarshalJSON(data []byte) error {
	value, err := proto.UnmarshalJSONEnum(AuthCodeSendScene_value, data, "AuthCodeSendScene")
	if err != nil {
		return err
	}
	*x = AuthCodeSendScene(value)
	return nil
}
func (AuthCodeSendScene) EnumDescriptor() ([]byte, []int) {
	return fileDescriptor_phone_f78d1882413fa7ec, []int{0}
}

// 发送验证码
type AuthCodeReq struct {
	CellphoneNum         *uint64            `protobuf:"varint,1,opt,name=cellphone_num,json=cellphoneNum" json:"cellphone_num,omitempty"`
	SendCase             *AuthCodeSendScene `protobuf:"varint,2,opt,name=send_case,json=sendCase,enum=hall.AuthCodeSendScene" json:"send_case,omitempty"`
	XXX_NoUnkeyedLiteral struct{}           `json:"-"`
	XXX_unrecognized     []byte             `json:"-"`
	XXX_sizecache        int32              `json:"-"`
}

func (m *AuthCodeReq) Reset()         { *m = AuthCodeReq{} }
func (m *AuthCodeReq) String() string { return proto.CompactTextString(m) }
func (*AuthCodeReq) ProtoMessage()    {}
func (*AuthCodeReq) Descriptor() ([]byte, []int) {
	return fileDescriptor_phone_f78d1882413fa7ec, []int{0}
}
func (m *AuthCodeReq) XXX_Unmarshal(b []byte) error {
	return xxx_messageInfo_AuthCodeReq.Unmarshal(m, b)
}
func (m *AuthCodeReq) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	return xxx_messageInfo_AuthCodeReq.Marshal(b, m, deterministic)
}
func (dst *AuthCodeReq) XXX_Merge(src proto.Message) {
	xxx_messageInfo_AuthCodeReq.Merge(dst, src)
}
func (m *AuthCodeReq) XXX_Size() int {
	return xxx_messageInfo_AuthCodeReq.Size(m)
}
func (m *AuthCodeReq) XXX_DiscardUnknown() {
	xxx_messageInfo_AuthCodeReq.DiscardUnknown(m)
}

var xxx_messageInfo_AuthCodeReq proto.InternalMessageInfo

func (m *AuthCodeReq) GetCellphoneNum() uint64 {
	if m != nil && m.CellphoneNum != nil {
		return *m.CellphoneNum
	}
	return 0
}

func (m *AuthCodeReq) GetSendCase() AuthCodeSendScene {
	if m != nil && m.SendCase != nil {
		return *m.SendCase
	}
	return AuthCodeSendScene_REGISTER
}

// 验证码接收
type AuthCodeRsp struct {
	ErrorCode            *uint64  `protobuf:"varint,1,opt,name=ErrorCode" json:"ErrorCode,omitempty"`
	ErrorMsg             *string  `protobuf:"bytes,2,opt,name=ErrorMsg" json:"ErrorMsg,omitempty"`
	XXX_NoUnkeyedLiteral struct{} `json:"-"`
	XXX_unrecognized     []byte   `json:"-"`
	XXX_sizecache        int32    `json:"-"`
}

func (m *AuthCodeRsp) Reset()         { *m = AuthCodeRsp{} }
func (m *AuthCodeRsp) String() string { return proto.CompactTextString(m) }
func (*AuthCodeRsp) ProtoMessage()    {}
func (*AuthCodeRsp) Descriptor() ([]byte, []int) {
	return fileDescriptor_phone_f78d1882413fa7ec, []int{1}
}
func (m *AuthCodeRsp) XXX_Unmarshal(b []byte) error {
	return xxx_messageInfo_AuthCodeRsp.Unmarshal(m, b)
}
func (m *AuthCodeRsp) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	return xxx_messageInfo_AuthCodeRsp.Marshal(b, m, deterministic)
}
func (dst *AuthCodeRsp) XXX_Merge(src proto.Message) {
	xxx_messageInfo_AuthCodeRsp.Merge(dst, src)
}
func (m *AuthCodeRsp) XXX_Size() int {
	return xxx_messageInfo_AuthCodeRsp.Size(m)
}
func (m *AuthCodeRsp) XXX_DiscardUnknown() {
	xxx_messageInfo_AuthCodeRsp.DiscardUnknown(m)
}

var xxx_messageInfo_AuthCodeRsp proto.InternalMessageInfo

func (m *AuthCodeRsp) GetErrorCode() uint64 {
	if m != nil && m.ErrorCode != nil {
		return *m.ErrorCode
	}
	return 0
}

func (m *AuthCodeRsp) GetErrorMsg() string {
	if m != nil && m.ErrorMsg != nil {
		return *m.ErrorMsg
	}
	return ""
}

// CheckAuthCodeReq 校验验证码请求
type CheckAuthCodeReq struct {
	SendCase             *AuthCodeSendScene `protobuf:"varint,1,opt,name=send_case,json=sendCase,enum=hall.AuthCodeSendScene" json:"send_case,omitempty"`
	Code                 *string            `protobuf:"bytes,2,opt,name=code" json:"code,omitempty"`
	Phone                *string            `protobuf:"bytes,3,opt,name=phone" json:"phone,omitempty"`
	XXX_NoUnkeyedLiteral struct{}           `json:"-"`
	XXX_unrecognized     []byte             `json:"-"`
	XXX_sizecache        int32              `json:"-"`
}

func (m *CheckAuthCodeReq) Reset()         { *m = CheckAuthCodeReq{} }
func (m *CheckAuthCodeReq) String() string { return proto.CompactTextString(m) }
func (*CheckAuthCodeReq) ProtoMessage()    {}
func (*CheckAuthCodeReq) Descriptor() ([]byte, []int) {
	return fileDescriptor_phone_f78d1882413fa7ec, []int{2}
}
func (m *CheckAuthCodeReq) XXX_Unmarshal(b []byte) error {
	return xxx_messageInfo_CheckAuthCodeReq.Unmarshal(m, b)
}
func (m *CheckAuthCodeReq) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	return xxx_messageInfo_CheckAuthCodeReq.Marshal(b, m, deterministic)
}
func (dst *CheckAuthCodeReq) XXX_Merge(src proto.Message) {
	xxx_messageInfo_CheckAuthCodeReq.Merge(dst, src)
}
func (m *CheckAuthCodeReq) XXX_Size() int {
	return xxx_messageInfo_CheckAuthCodeReq.Size(m)
}
func (m *CheckAuthCodeReq) XXX_DiscardUnknown() {
	xxx_messageInfo_CheckAuthCodeReq.DiscardUnknown(m)
}

var xxx_messageInfo_CheckAuthCodeReq proto.InternalMessageInfo

func (m *CheckAuthCodeReq) GetSendCase() AuthCodeSendScene {
	if m != nil && m.SendCase != nil {
		return *m.SendCase
	}
	return AuthCodeSendScene_REGISTER
}

func (m *CheckAuthCodeReq) GetCode() string {
	if m != nil && m.Code != nil {
		return *m.Code
	}
	return ""
}

func (m *CheckAuthCodeReq) GetPhone() string {
	if m != nil && m.Phone != nil {
		return *m.Phone
	}
	return ""
}

// CheckAuthCodeRsp 校验验证码应答
type CheckAuthCodeRsp struct {
	Result               *common.Result `protobuf:"bytes,1,opt,name=result" json:"result,omitempty"`
	XXX_NoUnkeyedLiteral struct{}       `json:"-"`
	XXX_unrecognized     []byte         `json:"-"`
	XXX_sizecache        int32          `json:"-"`
}

func (m *CheckAuthCodeRsp) Reset()         { *m = CheckAuthCodeRsp{} }
func (m *CheckAuthCodeRsp) String() string { return proto.CompactTextString(m) }
func (*CheckAuthCodeRsp) ProtoMessage()    {}
func (*CheckAuthCodeRsp) Descriptor() ([]byte, []int) {
	return fileDescriptor_phone_f78d1882413fa7ec, []int{3}
}
func (m *CheckAuthCodeRsp) XXX_Unmarshal(b []byte) error {
	return xxx_messageInfo_CheckAuthCodeRsp.Unmarshal(m, b)
}
func (m *CheckAuthCodeRsp) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	return xxx_messageInfo_CheckAuthCodeRsp.Marshal(b, m, deterministic)
}
func (dst *CheckAuthCodeRsp) XXX_Merge(src proto.Message) {
	xxx_messageInfo_CheckAuthCodeRsp.Merge(dst, src)
}
func (m *CheckAuthCodeRsp) XXX_Size() int {
	return xxx_messageInfo_CheckAuthCodeRsp.Size(m)
}
func (m *CheckAuthCodeRsp) XXX_DiscardUnknown() {
	xxx_messageInfo_CheckAuthCodeRsp.DiscardUnknown(m)
}

var xxx_messageInfo_CheckAuthCodeRsp proto.InternalMessageInfo

func (m *CheckAuthCodeRsp) GetResult() *common.Result {
	if m != nil {
		return m.Result
	}
	return nil
}

// GetBindPhoneRewardInfoReq 获取绑定手机可获得的奖励信息请求
type GetBindPhoneRewardInfoReq struct {
	Reserve              *uint32  `protobuf:"varint,1,opt,name=reserve" json:"reserve,omitempty"`
	XXX_NoUnkeyedLiteral struct{} `json:"-"`
	XXX_unrecognized     []byte   `json:"-"`
	XXX_sizecache        int32    `json:"-"`
}

func (m *GetBindPhoneRewardInfoReq) Reset()         { *m = GetBindPhoneRewardInfoReq{} }
func (m *GetBindPhoneRewardInfoReq) String() string { return proto.CompactTextString(m) }
func (*GetBindPhoneRewardInfoReq) ProtoMessage()    {}
func (*GetBindPhoneRewardInfoReq) Descriptor() ([]byte, []int) {
	return fileDescriptor_phone_f78d1882413fa7ec, []int{4}
}
func (m *GetBindPhoneRewardInfoReq) XXX_Unmarshal(b []byte) error {
	return xxx_messageInfo_GetBindPhoneRewardInfoReq.Unmarshal(m, b)
}
func (m *GetBindPhoneRewardInfoReq) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	return xxx_messageInfo_GetBindPhoneRewardInfoReq.Marshal(b, m, deterministic)
}
func (dst *GetBindPhoneRewardInfoReq) XXX_Merge(src proto.Message) {
	xxx_messageInfo_GetBindPhoneRewardInfoReq.Merge(dst, src)
}
func (m *GetBindPhoneRewardInfoReq) XXX_Size() int {
	return xxx_messageInfo_GetBindPhoneRewardInfoReq.Size(m)
}
func (m *GetBindPhoneRewardInfoReq) XXX_DiscardUnknown() {
	xxx_messageInfo_GetBindPhoneRewardInfoReq.DiscardUnknown(m)
}

var xxx_messageInfo_GetBindPhoneRewardInfoReq proto.InternalMessageInfo

func (m *GetBindPhoneRewardInfoReq) GetReserve() uint32 {
	if m != nil && m.Reserve != nil {
		return *m.Reserve
	}
	return 0
}

// GetBindPhoneRewardInfoRsp 获取绑定手机可获得的奖励信息应答
type GetBindPhoneRewardInfoRsp struct {
	Reward               *common.Money `protobuf:"bytes,1,opt,name=reward" json:"reward,omitempty"`
	XXX_NoUnkeyedLiteral struct{}      `json:"-"`
	XXX_unrecognized     []byte        `json:"-"`
	XXX_sizecache        int32         `json:"-"`
}

func (m *GetBindPhoneRewardInfoRsp) Reset()         { *m = GetBindPhoneRewardInfoRsp{} }
func (m *GetBindPhoneRewardInfoRsp) String() string { return proto.CompactTextString(m) }
func (*GetBindPhoneRewardInfoRsp) ProtoMessage()    {}
func (*GetBindPhoneRewardInfoRsp) Descriptor() ([]byte, []int) {
	return fileDescriptor_phone_f78d1882413fa7ec, []int{5}
}
func (m *GetBindPhoneRewardInfoRsp) XXX_Unmarshal(b []byte) error {
	return xxx_messageInfo_GetBindPhoneRewardInfoRsp.Unmarshal(m, b)
}
func (m *GetBindPhoneRewardInfoRsp) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	return xxx_messageInfo_GetBindPhoneRewardInfoRsp.Marshal(b, m, deterministic)
}
func (dst *GetBindPhoneRewardInfoRsp) XXX_Merge(src proto.Message) {
	xxx_messageInfo_GetBindPhoneRewardInfoRsp.Merge(dst, src)
}
func (m *GetBindPhoneRewardInfoRsp) XXX_Size() int {
	return xxx_messageInfo_GetBindPhoneRewardInfoRsp.Size(m)
}
func (m *GetBindPhoneRewardInfoRsp) XXX_DiscardUnknown() {
	xxx_messageInfo_GetBindPhoneRewardInfoRsp.DiscardUnknown(m)
}

var xxx_messageInfo_GetBindPhoneRewardInfoRsp proto.InternalMessageInfo

func (m *GetBindPhoneRewardInfoRsp) GetReward() *common.Money {
	if m != nil {
		return m.Reward
	}
	return nil
}

// BindPhoneReq 绑定手机请求
type BindPhoneReq struct {
	Phone                *string  `protobuf:"bytes,1,opt,name=phone" json:"phone,omitempty"`
	DymcCode             *string  `protobuf:"bytes,2,opt,name=dymc_code,json=dymcCode" json:"dymc_code,omitempty"`
	Passwd               *string  `protobuf:"bytes,3,opt,name=passwd" json:"passwd,omitempty"`
	XXX_NoUnkeyedLiteral struct{} `json:"-"`
	XXX_unrecognized     []byte   `json:"-"`
	XXX_sizecache        int32    `json:"-"`
}

func (m *BindPhoneReq) Reset()         { *m = BindPhoneReq{} }
func (m *BindPhoneReq) String() string { return proto.CompactTextString(m) }
func (*BindPhoneReq) ProtoMessage()    {}
func (*BindPhoneReq) Descriptor() ([]byte, []int) {
	return fileDescriptor_phone_f78d1882413fa7ec, []int{6}
}
func (m *BindPhoneReq) XXX_Unmarshal(b []byte) error {
	return xxx_messageInfo_BindPhoneReq.Unmarshal(m, b)
}
func (m *BindPhoneReq) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	return xxx_messageInfo_BindPhoneReq.Marshal(b, m, deterministic)
}
func (dst *BindPhoneReq) XXX_Merge(src proto.Message) {
	xxx_messageInfo_BindPhoneReq.Merge(dst, src)
}
func (m *BindPhoneReq) XXX_Size() int {
	return xxx_messageInfo_BindPhoneReq.Size(m)
}
func (m *BindPhoneReq) XXX_DiscardUnknown() {
	xxx_messageInfo_BindPhoneReq.DiscardUnknown(m)
}

var xxx_messageInfo_BindPhoneReq proto.InternalMessageInfo

func (m *BindPhoneReq) GetPhone() string {
	if m != nil && m.Phone != nil {
		return *m.Phone
	}
	return ""
}

func (m *BindPhoneReq) GetDymcCode() string {
	if m != nil && m.DymcCode != nil {
		return *m.DymcCode
	}
	return ""
}

func (m *BindPhoneReq) GetPasswd() string {
	if m != nil && m.Passwd != nil {
		return *m.Passwd
	}
	return ""
}

// BindPhoneRsp 绑定手机应答
type BindPhoneRsp struct {
	Result               *common.Result `protobuf:"bytes,1,opt,name=result" json:"result,omitempty"`
	Reward               *common.Money  `protobuf:"bytes,2,opt,name=reward" json:"reward,omitempty"`
	NewMoney             *common.Money  `protobuf:"bytes,3,opt,name=new_money,json=newMoney" json:"new_money,omitempty"`
	XXX_NoUnkeyedLiteral struct{}       `json:"-"`
	XXX_unrecognized     []byte         `json:"-"`
	XXX_sizecache        int32          `json:"-"`
}

func (m *BindPhoneRsp) Reset()         { *m = BindPhoneRsp{} }
func (m *BindPhoneRsp) String() string { return proto.CompactTextString(m) }
func (*BindPhoneRsp) ProtoMessage()    {}
func (*BindPhoneRsp) Descriptor() ([]byte, []int) {
	return fileDescriptor_phone_f78d1882413fa7ec, []int{7}
}
func (m *BindPhoneRsp) XXX_Unmarshal(b []byte) error {
	return xxx_messageInfo_BindPhoneRsp.Unmarshal(m, b)
}
func (m *BindPhoneRsp) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	return xxx_messageInfo_BindPhoneRsp.Marshal(b, m, deterministic)
}
func (dst *BindPhoneRsp) XXX_Merge(src proto.Message) {
	xxx_messageInfo_BindPhoneRsp.Merge(dst, src)
}
func (m *BindPhoneRsp) XXX_Size() int {
	return xxx_messageInfo_BindPhoneRsp.Size(m)
}
func (m *BindPhoneRsp) XXX_DiscardUnknown() {
	xxx_messageInfo_BindPhoneRsp.DiscardUnknown(m)
}

var xxx_messageInfo_BindPhoneRsp proto.InternalMessageInfo

func (m *BindPhoneRsp) GetResult() *common.Result {
	if m != nil {
		return m.Result
	}
	return nil
}

func (m *BindPhoneRsp) GetReward() *common.Money {
	if m != nil {
		return m.Reward
	}
	return nil
}

func (m *BindPhoneRsp) GetNewMoney() *common.Money {
	if m != nil {
		return m.NewMoney
	}
	return nil
}

// ChangePhoneReq 修改手机请求
type ChangePhoneReq struct {
	OldPhone             *string  `protobuf:"bytes,1,opt,name=old_phone,json=oldPhone" json:"old_phone,omitempty"`
	OldPhoneCode         *string  `protobuf:"bytes,2,opt,name=old_phone_code,json=oldPhoneCode" json:"old_phone_code,omitempty"`
	NewPhone             *string  `protobuf:"bytes,3,opt,name=new_phone,json=newPhone" json:"new_phone,omitempty"`
	NewPhoneCode         *string  `protobuf:"bytes,4,opt,name=new_phone_code,json=newPhoneCode" json:"new_phone_code,omitempty"`
	XXX_NoUnkeyedLiteral struct{} `json:"-"`
	XXX_unrecognized     []byte   `json:"-"`
	XXX_sizecache        int32    `json:"-"`
}

func (m *ChangePhoneReq) Reset()         { *m = ChangePhoneReq{} }
func (m *ChangePhoneReq) String() string { return proto.CompactTextString(m) }
func (*ChangePhoneReq) ProtoMessage()    {}
func (*ChangePhoneReq) Descriptor() ([]byte, []int) {
	return fileDescriptor_phone_f78d1882413fa7ec, []int{8}
}
func (m *ChangePhoneReq) XXX_Unmarshal(b []byte) error {
	return xxx_messageInfo_ChangePhoneReq.Unmarshal(m, b)
}
func (m *ChangePhoneReq) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	return xxx_messageInfo_ChangePhoneReq.Marshal(b, m, deterministic)
}
func (dst *ChangePhoneReq) XXX_Merge(src proto.Message) {
	xxx_messageInfo_ChangePhoneReq.Merge(dst, src)
}
func (m *ChangePhoneReq) XXX_Size() int {
	return xxx_messageInfo_ChangePhoneReq.Size(m)
}
func (m *ChangePhoneReq) XXX_DiscardUnknown() {
	xxx_messageInfo_ChangePhoneReq.DiscardUnknown(m)
}

var xxx_messageInfo_ChangePhoneReq proto.InternalMessageInfo

func (m *ChangePhoneReq) GetOldPhone() string {
	if m != nil && m.OldPhone != nil {
		return *m.OldPhone
	}
	return ""
}

func (m *ChangePhoneReq) GetOldPhoneCode() string {
	if m != nil && m.OldPhoneCode != nil {
		return *m.OldPhoneCode
	}
	return ""
}

func (m *ChangePhoneReq) GetNewPhone() string {
	if m != nil && m.NewPhone != nil {
		return *m.NewPhone
	}
	return ""
}

func (m *ChangePhoneReq) GetNewPhoneCode() string {
	if m != nil && m.NewPhoneCode != nil {
		return *m.NewPhoneCode
	}
	return ""
}

// ChangePhoneRsp 修改手机应答
type ChangePhoneRsp struct {
	Result               *common.Result `protobuf:"bytes,1,opt,name=result" json:"result,omitempty"`
	XXX_NoUnkeyedLiteral struct{}       `json:"-"`
	XXX_unrecognized     []byte         `json:"-"`
	XXX_sizecache        int32          `json:"-"`
}

func (m *ChangePhoneRsp) Reset()         { *m = ChangePhoneRsp{} }
func (m *ChangePhoneRsp) String() string { return proto.CompactTextString(m) }
func (*ChangePhoneRsp) ProtoMessage()    {}
func (*ChangePhoneRsp) Descriptor() ([]byte, []int) {
	return fileDescriptor_phone_f78d1882413fa7ec, []int{9}
}
func (m *ChangePhoneRsp) XXX_Unmarshal(b []byte) error {
	return xxx_messageInfo_ChangePhoneRsp.Unmarshal(m, b)
}
func (m *ChangePhoneRsp) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	return xxx_messageInfo_ChangePhoneRsp.Marshal(b, m, deterministic)
}
func (dst *ChangePhoneRsp) XXX_Merge(src proto.Message) {
	xxx_messageInfo_ChangePhoneRsp.Merge(dst, src)
}
func (m *ChangePhoneRsp) XXX_Size() int {
	return xxx_messageInfo_ChangePhoneRsp.Size(m)
}
func (m *ChangePhoneRsp) XXX_DiscardUnknown() {
	xxx_messageInfo_ChangePhoneRsp.DiscardUnknown(m)
}

var xxx_messageInfo_ChangePhoneRsp proto.InternalMessageInfo

func (m *ChangePhoneRsp) GetResult() *common.Result {
	if m != nil {
		return m.Result
	}
	return nil
}

func init() {
	proto.RegisterType((*AuthCodeReq)(nil), "hall.AuthCodeReq")
	proto.RegisterType((*AuthCodeRsp)(nil), "hall.AuthCodeRsp")
	proto.RegisterType((*CheckAuthCodeReq)(nil), "hall.CheckAuthCodeReq")
	proto.RegisterType((*CheckAuthCodeRsp)(nil), "hall.CheckAuthCodeRsp")
	proto.RegisterType((*GetBindPhoneRewardInfoReq)(nil), "hall.GetBindPhoneRewardInfoReq")
	proto.RegisterType((*GetBindPhoneRewardInfoRsp)(nil), "hall.GetBindPhoneRewardInfoRsp")
	proto.RegisterType((*BindPhoneReq)(nil), "hall.BindPhoneReq")
	proto.RegisterType((*BindPhoneRsp)(nil), "hall.BindPhoneRsp")
	proto.RegisterType((*ChangePhoneReq)(nil), "hall.ChangePhoneReq")
	proto.RegisterType((*ChangePhoneRsp)(nil), "hall.ChangePhoneRsp")
	proto.RegisterEnum("hall.AuthCodeSendScene", AuthCodeSendScene_name, AuthCodeSendScene_value)
}

func init() { proto.RegisterFile("phone.proto", fileDescriptor_phone_f78d1882413fa7ec) }

var fileDescriptor_phone_f78d1882413fa7ec = []byte{
	// 543 bytes of a gzipped FileDescriptorProto
	,
}
