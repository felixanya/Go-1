...
func (c *myCore) Init(e *structs.Exposer, param ...string) error {
	...
	if pbService != nil {
		if err := e.RPCServer.RegisterService(pbService, pbServerImp); err != nil {
			logrus.WithError(err).Error("注册RPC服务处理器失败")
			return err
		}
	}
	...
	if err := c.dispatchClientMsg(e.Exchanger); err != nil {
		logrus.WithError(err).Error("注册客户端Client消息处理器失败")
		return err
	}
	...
	err := InitServer()
	if err != nil {
		logrus.WithError(err).Error("初始化服务失败")
		return err
	}

	logrus.Debugf("server init succeed ...")
	return nil
}
